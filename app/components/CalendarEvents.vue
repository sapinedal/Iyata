<template>
    <div class="flex flex-col gap-4">
        <div class="flex items-center justify-between">
            <h2 class="text-lg font-bold text-[#0D0D0D]">{{ title }}</h2>
        ...
        <div class="flex flex-col gap-3">
            <div 
                ...
                :key="index"
                class="flex gap-3 p-3 hover:bg-[#E3E3E3]/20 rounded-xl transition-all duration-200 cursor-pointer group"
            >

                <div class="flex flex-col items-center justify-center bg-[#E3E3E3] rounded-xl p-3 min-w-[60px]">
                    <span class="text-xs font-medium text-[#727272] uppercase">{{ event.month }}</span>
                    <span class="text-2xl font-bold text-[#0D0D0D]">{{ event.day }}</span>
                </div>
                
                <div class="flex-1 flex flex-col justify-center gap-1">
                    <h3 class="text-sm font-semibold text-[#0D0D0D] group-hover:text-[#2A2A2A]">
                        {{ event.title }}
                    </h3>
                    <div class="flex items-center gap-2">
                        <span class="text-xs text-[#727272]">{{ event.time }}</span>
                        <span v-if="event.location" class="text-xs text-[#727272]">• {{ event.location }}</span>
                    </div>
